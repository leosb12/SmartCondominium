# backendapi/analytics/services.py
...
# ---- Tus services separados (no se tocan) ----
from .services_morosidad_areas import (
    morosidad_service,
    areas_uso_service,
    diagnostico_morosidad,
)
from .services_seguridad_export import (
    seguridad_service,
    diagnostico_seguridad,
)
...
def _to_csv_bytes(rows: List[Dict[str, Any]], header: Optional[List[str]]) -> bytes:
    """
    Crea CSV estable. Si header es None, arma encabezado con la unión de keys.
    """
    sio = StringIO()
    w = csv.writer(sio)

    # Determinar encabezado si no se provee
    if header is None:
        keys: List[str] = []
        for r in rows or []:
...
def _fallback_seguridad_kpis(seg: Dict[str, Any]) -> Dict[str, Any]:
    """
    Si seguridad_service no marca horas calientes por z-score, mostramos
    top por conteo simple para que el dashboard no quede vacío.
    """
    k = seg.get("kpis")
    if isinstance(k, dict) and (k.get("horas_calientes_autos") or k.get("horas_calientes_personas")):
        return k
...
# ============================================================
# Dashboard (orquesta los 3 módulos y aplica fallback de seguridad)
# ============================================================
...
# ============================================================
# Exportación (CSV) reusando los services ya separados
# ============================================================
def export_service(
    tipo: str,
    query_params,
